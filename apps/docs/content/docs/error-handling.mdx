...
| `BAD_REQUEST` | Router | Invalid JSON body, `args` is not an array, or batch size exceeds `maxBatchSize` |
| `UNAUTHORIZED` | Your code | Throw to signal authentication failure — maps to HTTP 401 |
| `FORBIDDEN` | Your code | Throw to signal authorization failure — maps to HTTP 403 |
